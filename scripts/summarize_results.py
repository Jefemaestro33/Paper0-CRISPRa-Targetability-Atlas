...
def main():
    ...
    key_counts = {
        "n_genes": int(atlas.gene.nunique()),
        "n_panel": int(panel.gene.nunique()),
        "n_runs": int(len(runs)),
        "n_studies": 3,
        "n_contexts": int(len(STATES)),
        "n_targeting_classes": int(len(CAS_MACRO)),
        "n_crispra_configurations": 6,
        "n_tss_definitions": 3,
        "n_matched_depth_peak_callers": 2,
        "permutation_resamples": 10000,
        "permutation_seed": 1729,
    }
    macros={
        "NGene":key_counts["n_genes"],
        "NGeneFormatted":f"{key_counts['n_genes']:,}",
        "NPanel":key_counts["n_panel"],
        "NRun":key_counts["n_runs"],
        "NPermutationResamplesFormatted":f"{key_counts['permutation_resamples']:,}",
        "PermutationSeed":key_counts["permutation_seed"],
    }
    ...
    for gene in ("Tfeb","Tfe3"):
        ...
        macros[f"{macro_gene}ApprisTSS"]=int(gene_tss["appris_principal"])
        macros[f"{macro_gene}LegacyTSS"]=int(gene_tss["legacy_most5_basic"])
        macros[f"{macro_gene}ApprisOffset"]=abs(int(gene_tss["appris_principal"]-gene_tss["ensembl_canonical"]))
    ...
    stability=pd.read_csv(ROOT/"analysis_stats/therapeutic_gene_stability.tsv",sep="\t")
    for gene in ("Tfeb", "Tfe3"):
        macro_gene = "TfeThree" if gene == "Tfe3" else gene
        gene_stability=stability[stability.gene==gene]
        for column,label in (("appris_principal_tss","Appris"),("legacy_most5_basic_tss","Legacy")):
            values=gene_stability[column].astype(str).str.lower().eq("true")
            macros[f"{macro_gene}{label}ContextsN"]=int(values.sum())
    ...
    key_counts_path=ROOT/"analysis_stats/key_counts.json"
    key_counts_path.write_text(json.dumps(key_counts, indent=2, sort_keys=True) + "\n")
    print(f"Wrote {macro_path}, {audit}, and {key_counts_path}")
