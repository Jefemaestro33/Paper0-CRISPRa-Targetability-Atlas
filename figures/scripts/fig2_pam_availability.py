...
def main():
    ...
    for axis, series, title in [(fig.add_subplot(grid[0,0]), genome_pam, "A  Genome-wide PAM/protospacer coverage"), (fig.add_subplot(grid[0,1]), panel_pam, "B  Locked 55-gene panel")]:
        ...
        for i, value in enumerate(values): axis.text(i, value+.5, f"{value:.1f}%", ha="center", fontsize=8, weight="bold")
        axis.set_xticks(range(5)); axis.set_xticklabels([CAS_LABELS[c].split("\n")[0] for c in CAS_ORDER], fontsize=6.5)
        axis.set_ylabel("Promoters with ≥1 candidate (%)", fontsize=7.5); axis.set_ylim(0, 105)
    ...
    ax.set_title("C  Guide-site support by dataset context", loc="left", weight="bold", fontsize=9); ax.legend(fontsize=5.5, ncol=2); ax.spines[["top","right"]].set_visible(False)
